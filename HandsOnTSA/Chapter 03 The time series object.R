# Chapter 03 The time series object
...
# -------- Code Chank 19 --------
# to arrange 'date' first before conversion
...
daily_ts <- ts(daily_df$y, start = c(year(start_date), yday(start_date)),
               frequency = 365)
ts_info(daily_ts)

daily_leap_ts <- ts(daily_df$y,
                    start = c(year(start_date), yday(start_date)),
                    frequency = 365.25)

ts_info(daily_leap_ts)
