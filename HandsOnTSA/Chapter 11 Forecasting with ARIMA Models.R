...
# The AR process ----------------------------------------------------------
# ar has to be between -1 and 1, otherwise it is 'trending'
# -------- Code Chank 11 --------
set.seed(12345)
...
# If the ACF is cut off at lag q and the PACF function tails 
# off, we can conclude that the process is an MA(q).
# -------- Code Chank 18 --------
par(mfrow=c(1,2))
acf(ma2)
pacf(ma2)
...
arma <- arima.sim(model = 
                    list(order(1,0,2), ar = c(0.7), ma = c(0.5,-0.3)), 
                  n = 500)
# -------- Code Chank 20 --------
ts_plot(arma, 
...
# factor variable or categorical variable, seasonal component
df$month <- factor(month(df$date, label = TRUE), ordered = FALSE)

# trend, moving with time by one index unit
df$trend <- 1:nrow(df)
